...
The Bioconductor ecosytem integrates the methods of microbiome data science
...
Common alternatives include phyloseq [@mcmurdie2012]&mdash;with its extension
speedyseq&mdash;, which is also part of Bioconductor, and the command-line
toolkits Mothur [@schloss2009], QIIME 2 [@bolyen2019], and Anvi'o [@eren2021].
Each of these frameworks was designed to tackle recurring challenges in
microbiome data science&mdash;a field where no single solution yet provides the
most complete, coherent, and efficient approach.

Compared to its alternatives, the (Tree)SE framework relies on a more compact,
efficient data structure that is fully interoperable with the larger
Bioconductor ecosystem. This way, it provides an evidence-based, accessible
solution for developing end-to-end microbiome analysis workflows that can
integrate concurrent Bioconductor development efforts, without being limited
exclusively to methods for microbiome data analysis.
...
# Import results
df <- read.table("data/benchmark_results.csv", sep = ",", header = TRUE)
...
wrapped into a TreeSE data container,
...
# Load packages
pkgs <- c("ggpubr", "miaViz", "patchwork", "scater")
temp <- sapply(pkgs, function(x) {
    suppressMessages(require(x, character.only = TRUE))
})
...
p_alpha2 <- plotColData(
    ...
    stat_cor(method = "kendall", cor.coef.name = "tau", label.x.npc = 0.9) +
