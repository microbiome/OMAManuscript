...
heterogeneous data across multiple data modes into reproducible workflows [@ragan-kelley2013;@callahan2016;@holmes2019].
The unique statistical properties and complexity of microbiome data
necessitate specialized approaches in their analysis
[@moreno-indias2021; @jeganathan2021; @willis2019; @bastiaanssen_bugs1_2023; @bastiaanssen_bugs2_2023], yet the diversity of
...
The (Tree)SE framework is one of several approaches to microbiome analysis.
Within Bioconductor, the well-established phyloseq [@mcmurdie2012] framework and
its extension speedyseq provide a mature ecosystem for 16S amplicon data
analysis. TreeSE accommodates all core components of phyloseq objects, including
abundance tables, taxonomic annotations, phylogenetic trees, reference
sequences, and sample metadata, while extending the data container to support
the additional elements discussed in @sec-infra. Other widely used frameworks
include the command-line toolkits Mothur [@schloss2009], QIIME 2 [@bolyen2019],
and Anvi'o [@eren2021], each supported by active user and developer communities
and primarily focused on taxonomic microbiome analyses. In contrast, TreeSE
builds on the shared infrastructure, enabling direct interoperability
with the broader Bioconductor ecosystem and facilitating hierarchical and
multi-modal analyses, while leveraging statistical methods developed beyond
microbiome research.

This integrative approach to bioinformatics is also found in the Python library
scikit-bio, which provides general methods for various omics modalities
[@aton2025]. While relatively efficient, its infrastructure for microbiome
analysis is not yet as extensive and does not leverage integrative data
containers such as TreeSE. Nonetheless, certain scikit-bio modules for community
diversity estimation are available through the larger QIIME 2 framework for
microbiome analysis.

Based on an extensive benchmark of routine operations in microbiome analysis
applied to subsets of the Metalog database [@kuhn2026], the (Tree)SE framework
shows substantial performance gains over alternatives (@fig-benchmarking and
Supplementary Figure 1). While speedyseq performs faster in certain operations,
TreeSE tends to scale more efficiently in terms of execution time and allocated
memory as feature and sample sizes increase. As most operations exceeded
resource limits for very large sample subsets, future development will need to
focus on further optimizing these routine operations in the (Tree)SE framework,
*e.g.*, by executing costly computations in C++ and by leveraging the built-in
support for sparse and delayed matrices in SE-based data containers, which can
significantly improve method efficiency [@huber2015]. To facilitate migration
from alternative frameworks, the OMA book provides conversion tables and utility
functions to convert between TreeSE and external formats.
...
formats, including BIOM [@mcdonald2012], MetaPhlAn
v2&ndash and HUMAnN (multiple versions) [@beghini2021], Mothur [@schloss2009], QIIME 2
[@bolyen2019], and TAXPASTA [@beber2023]. These importers bridge the gap
...
and the R for Mass Spectrometry (R4MS) project provides many interoperable tools.
...
We also converted a large-scale compilation of resistome profiles in 8,972
metagenome samples [@lee2023] into the TreeSE format to support
research and training in this area of microbiome analysis.
...
Key feature of the ecosystem is the support for integrating 
microbial abundance data with potentially complex 
combinations other omics modalities, including 
resistomes [@salehi2025], 
transcriptomes, proteomes, or metabolomes [@hintikka2021; @mangnier2025], 
host genomes [@qin2022], and 
environmental exposomes (@fig-downstream_analyses). 
By leveraging standardized data containers (Fig.~\ref{fig:datacontainers}), 
users can apply a growing number of methods for multi-omics analysis in microbiome research. 
The containers reduce the need for manual data wrangling and the risk of errors (*e.g.*, sample
mismatches or data formatting), support modular and reproducible
workflows, and facilitates incorporation of further downstream tools 
such as mixOmics [@rohart2017]. Standard integrative approaches in microbiome
research can be broadly categorized into three groups: association-based approaches, latent variable models, and Machine Learning (ML) prediction [@sankaran2019; @sankaran2019b; @sherwani2025; @singh2019].
Association-based methods quantify relations between datasets.
Whereas the cross-correlation analysis can be used to capture direct
associations of samples or features between two datasets and are often straightforward 
to interpret, they do not explicitly account for the potential cooperation and 
co-variation between microbial features. Methods such as 
anansi (Bioconductor package) can supervise
such analyses by incorporating known functional relations. Latent variable methods such
as multi-omics factor analysis (MOFA+) [@argelaguet2020],
joint robust PCA (Joint-RPCA) [@vargas2026] and multiblock partial least
...
Machine learning-based methods such as IntegratedLearner [@mallick2024] can identify and 
leverage the dependencies between omics layers towards improved prediction performance.
The available methodologies are rapidly increasing. Unified data science frameworks, 
such as the one proposed here, can accelerate methods development, benchmarking, and standardization.
...
microbe-set enrichment analysis [@kou2020; @pham2025]. The recent adoption of interoperable
...
but also with external microbiome data analysis pipelines using tools such as bioBakery [@beghini2021],
and foreign platforms through language-specific interfaces,
...
questions. The Bioconductor ecosystem naturally aligns with these goals through
...
(OSCA book [@amezquita2020]) and spatial transciptomics (OSTA Bioconductor book).
These resources
...
trustworthiness, and explainability of analyses. We are here
documenting the open data science ecosystem for multi-modal microbiome research
...
Future extensions could include temporal, spatial, and multi-omic
methods. Moreover, the field could benefit from improved support for 
FAIR data practices and emerging community standards, *e.g.*, 
Minimum Information about any (x) Sequence (MIxS) [@field2008],
reporting guidelines for Human Microbiome Research (STORMS) [@mirzayi2021], or
standards for technical reporting in environmental and host-associated microbiome studies (STREAMS) [@kelliher2025].
The user community could also benefit from an extended collection of end-to-end 
case studies demonstrating how different example analyses can be conducted in practice, 
from original source data to results.
...
supported by the University of Turku Graduate School. We would
...
Ruuskanen; Mahkameh Salehi. We thank Christopher Quince and
Kihyun Lee for providing the resistome profiling data,
Justin Silverman for philr support, and Ben Allen and Sandy Rogers for MGnifyR support.
