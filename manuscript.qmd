...
Common alternatives include phyloseq [@mcmurdie2012]&mdash;and its extension
speedyseq&mdash;, which is also part of Bioconductor, and the command-line
platforms QIIME 2 [@bolyen2019], Mothur [@schloss2009] and Anvi'o [@eren2021].
...
integrate concurrent development efforts in Bioconductor, without being limited
...
The (Tree)SE framework provides a substantial gain in performance over alternative
frameworks (@fig-benchmarking and Supplementary Figure 1). While
speedyseq is faster in certain operations, TreeSE
...
#|   operations.** TreeSE, QIIME 2 and phyloseq&mdash;with its extension
#|   speedyseq&mdash;are three popular frameworks for microbiome analysis. We
#|   benchmarked them for five routine operations applied to random subsets of
#|   features (rows) and samples (columns) from the Metalog database [@kuhn2026].
#|   Both execution time (s) and allocated memory (MB) were measured as the mean
#|   value over ten replicates generated. Errorbars show the 95% CI of the estimated
#|   performance metric. The benchmarking was conducted in R using the bench library
#|   with 4 CPU cores and 16 GB RAM. The source code for the benchmarking
#|   analysis is openly available [@benedetti2026b]."
#| fig-width: 14
#| fig-height: 14
