...
\end{center}
\captionof{figure}{\textbf{Bioconductor data containers for microbiome data.}
\textbf{(a) SummarizedExperiment (SE)} \cite{huber2015}.
\textbf{(b) TreeSummarizedExperiment (TreeSE)} supports hierarchical, multi-table
microbiome analysis. A data object can accommodate: i) \textit{assays} that describe
the abundances of features in each sample (\textit{e.g.} taxonomic units, resistance
genes, or predicted functions), ii) \textit{colData} with tabular metadata on each
sample, iii) \textit{rowData} with tabular metadata on each feature (\textit{e.g.}, taxonomic mappings), and iv) tree information describing feature hierarchies, such as
...
derived versions (\textit{e.g.}, statistical transformations, different taxonomic
...
\cite{huang2021}. \textbf{(c) MultiAssayExperiment (MAE)} facilitates the integration of interlinked datasets that may represent different omics modalities
...
#|   measured as the mean value over ten replicates generated. Errorbars show
#|   the 95% confidence interval of the performance estimate. The benchmarking
#|   was conducted in R using the bench library with 4 CPU cores and 16 GB RAM.
#|   QIIME 2 lacks allocated memory estimates because memory cannot be monitored
#|   for external processes. The source code for the benchmarking analysis is
#|   openly available [@benedetti2026b]."
#| fig-width: 12
#| fig-height: 14
...
plot_bench <- function(df, bench.var, error.var = "SE"){
    ...
    p <- ggplot(df, aes(x = cols, y = Mean, colour = object)) +
        ...
        scale_colour_manual(
            labels = classes,
            values = c("black", "mediumblue", "firebrick", "tomato"),
            drop = FALSE
        )
...
v2&ndash;4, HUMAnN v3) [@mciver2017], Mothur [@schloss2009], QIIME 2
...
**Association analysis.** Differential Abundance Analysis (DAA)
...
mia datasets are accessible via mia [@benedetti2026a]. Metagenomic samples from
the Metalog database is accessible via mia [@kuhn2026].

## Code availability

This article presented v1.1.1 of the OMA book (2026-05-07), dependent on R v4.6
and Bioconductor v3.24. Scripts and results of the benchmarking analysis are
openly available [@benedetti2026b].
